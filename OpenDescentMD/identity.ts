 ...
 *
 * We also derive X25519 keys from the Ed25519 keys for Diffie-Hellman
 * key exchange (used in message encryption).
 */
 ...
 *
 * In production, this would use libsodium's crypto_sign_keypair()
 * and crypto_sign_ed25519_pk_to_curve25519() for key conversion.
 * For the prototype, we use Node's built-in crypto with Ed25519.
 */
export function generateIdentity(displayName?: string): Identity {
  ...
  // For the prototype, we use the Ed25519 keys directly.
  // In production, derive X25519 keys using libsodium:
  //   crypto_sign_ed25519_pk_to_curve25519(ed25519_pk)
  //   crypto_sign_ed25519_sk_to_curve25519(ed25519_sk)
  //
  // For now, generate separate X25519 keys for encryption
 ...
 * This is a base58-encoded hash of the public key, similar to Bitcoin addresses.
 */
export function publicKeyToPeerId(publicKey: Uint8Array): string {
  const hash = crypto.createHash('sha256').update(publicKey).digest();
  return base58Encode(hash.subarray(0, 20)); // 20 bytes = 160-bit ID
}

/**
 * Save identity to disk (encrypted with a passphrase).
 *
 * CRITICAL: The private key file is the user's entire identity.
 * In a real app, this would use OS keychain or hardware security module.
 */
export function saveIdentity(identity: Identity, filePath: string, passphrase: string): void {
  ...
  // Encrypt with passphrase using AES-256-GCM
...
export function createPeerProfile(identity: Identity, addresses: string[]): PeerProfile {
  ...
  // Sign the profile (excluding the signature field)
  const dataToSign = new TextEncoder().encode(JSON.stringify(profileData));
