 ...
 * 5. Send: encrypted data + ephemeral public key + nonce
 *
 * Recipient:
 ...
 * @param recipientEncryptionPubKey - Recipient's X25519 public key (DER encoded)
 * @returns Encrypted payload with all data needed for decryption
 */
 ...
 * @param ourEncryptionPrivateKey - Our X25519 private key (DER encoded)
...
export function decryptFromPeer(
  ...
  // Perform Diffie-Hellman with sender's ephemeral key
  const sharedSecret = performDH(
...
function deriveKey(sharedSecret: Buffer, context: string): Buffer {
  ...
  return crypto.hkdfSync(
    'sha256',
    sharedSecret,
    Buffer.from('decentranet-salt'), // In production, use a proper salt
    Buffer.from(context),
    32, // 256-bit key
