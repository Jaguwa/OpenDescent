...
/** Ed25519 public key — this IS the user's identity */
export type PeerId = string; // base58-encoded public key
...
export interface Identity {
  ...
  /** X25519 private key for encryption (derived from Ed25519) */
...
export type ContentId = string; // CIDv1 string

/** A single shard of a larger piece of content */
export interface Shard {
  /** Unique ID for this shard */
  shardId: string;
  /** The ContentId of the original complete content */
  contentId: ContentId;
...
export interface ContentManifest {
  ...
  /** Timestamp */
  createdAt: number;
  /** Author's signature over the manifest */
  signature: Uint8Array;
}
...
/** All message types that can be sent over the P2P network */
...
export interface NodeConfig {
  ...
  dataDir: string;
  /** Path to store/load identity keys */
  identityPath: string;
  /** Max storage to dedicate to network (bytes) */
  maxStorageBytes: number;
  /** Max number of shards to store for other peers */
  maxShards: number;
  /** Enable relay service for other peers */
  enableRelay: boolean;
  /** TTL for stored messages awaiting delivery (seconds) */
